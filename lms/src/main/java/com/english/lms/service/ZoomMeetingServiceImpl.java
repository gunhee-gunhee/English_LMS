...
//確認（後で消す。）
	...
	System.out.println("className : " + className);
		...
		/**lms_class table 保存
		 * 
		 * student_num, teacher_num, week_days, text_num, start_date, start_time(:classStartTime), end_time(:classEndTime), class_month
		 * 
		 * */
...
System.out.println("classMonth : " + classMonth);
		...
		ClassEntity classEntity = ClassEntity.builder()
				.studentNum(classDTO.getStudentNum())
				.teacherNum(classDTO.getTeacherNum())
				.weekDays(classDTO.getWeekDays())
				.textNum(classDTO.getTextNum())
		 ...
		 * 
		 * class_num, class_type(:classType), class_name, class_date, start_time(:classStartTime), end_time(:classEndTime), zoom_link(:joinUrl), teacher_link(:teacherUrl),
		 * 
		...
		List<String> weekDayList = Arrays.stream(weekDays.split(","))
		    .map(String::trim)
